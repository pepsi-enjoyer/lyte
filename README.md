...
Hermes is a lightweight, read-only DOCX and XLSX viewer built with Rust and Tauri. The goal is simple: open Office documents quickly, render useful content, and avoid the overhead of launching a full editor when you only need to read.
...
- Hermes opens `.docx` and `.xlsx` files for reading only. It does not edit or save documents.
